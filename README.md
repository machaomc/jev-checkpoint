...
A complete Codex plugin: a focused review skill plus a bundled local MCP server. Codex implements, diagnoses and tests; TypeSafe's Jev supplies independent quality signals. Each task checkpoint gets at most **two outgoing API attempts per MCP process**, including failures.

This is a community project by [machaomc](https://github.com/machaomc), not an official OpenAI or TypeSafe product. It does not replace Codex's model, automatically route every prompt, or promise a fixed speedup or saving.

## Install from the GitHub URL

Requires a current Codex desktop/CLI with plugin marketplace support, **Node.js 20+ on the Codex host's PATH**, and your own **TypeSafe API key**. No npm install or build is required to use the plugin: the server and its dependencies are bundled in Git.
...
Then start a **new Codex task**. Ask:
...
1. Codex implements and runs relevant checks.
2. Jev evaluates the focused diff and necessary context.
3. Codex inspects weak or uncertain dimensions against the actual code.
4. If a justified fix exists, Codex makes it, validates and uses the one remaining follow-up.
...
Optional `files` is an array of `{ "path": "src/file.ts", "content": "..." }`. Keep the task and scope stable within a checkpoint. Follow-ups supply the current implementation; no `previousEvaluation` payload is needed because comparisons are local.
...
To update, refresh the market and reinstall, then start a new task:
...
`npm run build` bundles the server and dependency license notices. Commit generated `plugins/jev-checkpoint/dist/` alongside source. CI rebuilds and checks for drift on Linux, macOS and Windows.
...
This GitHub repository is a **custom Codex plugin marketplace** using `.agents/plugins/marketplace.json`. Adding its URL makes its plugin available to that user; it does not submit or list it in OpenAI's official public directory. Current official submission guidance expects a public HTTPS MCP endpoint for the usual With MCP route; this release intentionally uses local stdio. See [OpenAI packaging documentation](https://developers.openai.com/plugins/build/plugins).

The repository nests the plugin in `plugins/jev-checkpoint`; use the marketplace URL commands above. A generic “install a skill from URL” flow may install just the skill and omit the MCP server. Other clients are not claimed as tested.
