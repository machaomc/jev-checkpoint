...
try {
  await client.connect(new StdioClientTransport({ command: configuration.command,
    args: configuration.args.map(arg => arg.replaceAll('${PLUGIN_ROOT}', root)),
    cwd: resolve(root, configuration.cwd), env: { JEV_API_KEY: '', JEV_CHECKPOINT_CONFIG: join(dir, 'missing.json') }, stderr: 'pipe' }));
